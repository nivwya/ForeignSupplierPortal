...
.container-grid { display: grid; grid-template-columns: 1fr 1fr; grid-template-areas: "admin admin" "buttons buttons"; gap: 20px; margin: auto; padding: 20px; max-width: 700px; }
.admin { grid-area: admin; }
...
<div class="header">
  <div class="left-section">
    ...
    <h1>Admin Portal - AlMulla Group</h1>
  </div>
...
<form method="POST" action="{{ route('admin.profile.save', ['admin' => $admin->id]) }}">
    @csrf
    <div class="container-grid">
        <!-- Admin Info -->
        <div class="box admin">
            <div class="section-header">Admin Details</div>
            <div class="line"></div>
            <div class="form-grid">
                <label>ID:</label>
                ...
                <label>Email:</label>
                <input type="email" name="admin_email" value="{{ $admin->admin_email }}" readonly>

                <label>Company Code:</label>
                <input type="text" name="company_code" value="{{ $admin->company_code ?? '' }}">

                <label>Employee ID:</label>
                <input type="text" name="emp_id" value="{{ $admin->emp_id ?? '' }}">

                <label>PO:</label>
                <input type="text" name="po" value="{{ $admin->po ?? '' }}">
                ...
                <select name="status" required>
                    <option value="" disabled {{ $admin->status ? '' : 'selected' }}>Select Status</option>
                    <option value="active" {{ $admin->status == 'active' ? 'selected' : '' }}>Active</option>
                    <option value="not active" {{ $admin->status == 'not active' ? 'selected' : '' }}>Not Active</option>
                </select>
            </div>
        </div>
        <!-- Buttons -->
