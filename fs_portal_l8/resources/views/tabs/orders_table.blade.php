    ...
    <table id="orders-table">
        ...
        <tbody>
            ...
            <tr data-order-id="{{ $order['id'] }}">
                <td>
                    <a href="#" class="order-link" data-order="{{ $order->purchase_doc_no }}">
                        {{ $order->purchase_doc_no }}
                    </a>
                </td>
                <td>{{ $order->purchase_order_dt }}</td>
                <td>{{ $order->delivery_date }}</td>
                <td>{{ $order->company_code }}</td>
                <td>{{ $order->desc_purchase_org }}</td>
                ...
                <td>
                    @if($order->po_pdf)
                        <a href="{{ Storage::url($order->po_pdf) }}" target="_blank"> <i class="fa fa-file-pdf-o" style="font-size:15px;color:red;margin-right:4px"></i>{{$order->purchase_doc_no}}</a>
                    @else
