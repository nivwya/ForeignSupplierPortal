...
<form method="POST" action="{{ route('deliveries.batchSave', ['orderId' => $order->id]) }}" id="batch-save-form">
    @csrf
    <div class="po-items-container">
        <table style="width:100%; border-collapse:collapse; border:1px solid #b3c1c7;">
            ...
            @foreach($order->items as $poItem)
                @php
                    $deliveryItems = \App\Models\DeliveryItem::where('purchase_order_item_id', $poItem->id)
                        ->orderBy('created_at')->get();
                    $totalSupplied = $deliveryItems->sum('quantity_supplied');
                    $qtyRemaining = $poItem->quantity - $totalSupplied;
                @endphp

                @foreach($deliveryItems as $loopIndex => $item)
                    <tr @if($item->quantity_supplied > 0) style="background:#f7f7f7;" @endif>
                        <td style="border:1px solid #b3c1c7;">{{ $order->order_number }}</td>
                        <td style="border:1px solid #b3c1c7;">{{ $poItem->line_item_no }}</td>
                        <td style="border:1px solid #b3c1c7;">{{ $poItem->item_description }}</td>
                        <td style="border:1px solid #b3c1c7;">{{ $poItem->quantity }}</td>
                        <td style="border:1px solid #b3c1c7;">
                            {{ max($poItem->quantity - $deliveryItems->sum('quantity_supplied') + ($item->quantity_supplied ?? 0), 0) }}
                        </td>
                        <td style="border:1px solid #b3c1c7;">{{ $item->expected_delv_date ?? '-' }}</td>
                        <td style="border:1px solid #b3c1c7;">{{ $poItem->uom }}</td>
                        <td style="border:1px solid #b3c1c7;">
                            @if(auth()->user()->hasRole('vendor') && ($item->quantity_supplied == 0 || $item->quantity_supplied === null))
                                ...
                                <input type="number"
                                    ...
                                    min="0"
                                    max="{{ $poItem->quantity - $deliveryItems->where('id', '!=', $item->id)->sum('quantity_supplied') }}"
                        ...
                        <td style="border:1px solid #b3c1c7;">
                            @if(auth()->user()->hasRole('vendor') && ($item->quantity_supplied == 0 || $item->quantity_supplied === null))
                                <input type="date"
                                    ...
                                    data-item-id="{{ $item->id }}"
                                    value="{{ $item->supply_date ? \Carbon\Carbon::parse($item->supply_date)->format('Y-m-d') : '' }}">
                            @else
                                {{ $item->supply_date ? \Carbon\Carbon::parse($item->supply_date)->format('m/d/Y') : '-' }}
                            @endif
                        </td>
                        <td style="border:1px solid #b3c1c7;">{{ $item->qty_received_by_amg ?? '-' }}</td>
                        <td style="border:1px solid #b3c1c7;">{{ $item->amg_received_date ? \Carbon\Carbon::parse($item->amg_received_date)->format('m/d/Y') : '-' }}</td>
                        <td style="border:1px solid #b3c1c7;">
                            @if(auth()->user()->hasRole('vendor') && $item->quantity_supplied > 0 && $qtyRemaining > 0 && $loop->last)
                                <button type="button"
        ...
        @if($order->items->flatMap(function($poItem) {
            return \App\Models\DeliveryItem::where('purchase_order_item_id', $poItem->id)
                ->where(function($query) {
                    $query->whereNull('quantity_supplied')->orWhere('quantity_supplied', 0);
                })->get();
        })->count() > 0 && auth()->user()->hasRole('vendor'))
            <div style="text-align:right; margin-top:16px;">
                <button type="submit" class="save-all-btn" style="background:#4caf50; color:#fff; border:none; border-radius:6px; padding:8px 22px; font-size:1rem; font-weight:600; cursor:pointer;">
                    Save All
                </button>
            </div>
        @endif
...
document.getElementById('batch-save-form').addEventListener('submit', function(e) {
    ...
    qtyInputs.forEach(function(qtyInput, idx) {
        ...
        // If either field is filled, validate the pair
